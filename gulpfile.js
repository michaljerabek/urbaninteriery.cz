/*jshint esnext: true, evil: true, browser: true, devel: true, jquery: true, node: true*/

/* Sestavení (build) šablony a automatizace: Nastavení tasků pro Gulp.
 *
 ...
 *  7. CSS (Spojení CSS souborů a PostCSS – prefixy, CSS Custom Properties, @media px -> em.)
...
const gFile = file;
/*Promise pro .pipe*/
const gPromise = requireModule("gulp-promise");
//"jQuery pro Node.js"
...
function watch() {
    ...
    gulp.watch([
        PATHS.CSS_LOAD,
        PATHS.CSS_FILES_WATCH,
        PATHS.CSS_LIBS_FILES_WATCH
    ], css);

    gulp.watch([
        PATHS.JS_LOAD,
        PATHS.JS_FILES_WATCH,
        PATHS.JS_LIBS_FILES_WATCH
    ], js);
...
exports.build = gulp.parallel(css, js, prod);
...
function prod() {
    ...
    return switchToEnv("prod");
}

function switchToEnv(env) {

    return gulp.src(path.resolve(PATHS.HTML_INPUT, "*.html"))
 ...
 * last (flag): zachovat pouze poslední uložené dokumenty
 *
 * Seznam modulů: gulp yarg gulp-replace gulp-cheerio gulp-promise
 */
...
function copy(cb) {
    ...
    const gpromise = new gPromise();

    gpromise.makePromises(["copy"], () => cb());
    ...
    if (yargs.argv.component || yargs.argv.c) {
                ...
                .pipe(
                    gReplace(getHTMLComponentRegExp(source), match => {
                        ...
                        if (replaceElement) {
                            ...
                            replaceHTMLElementWithComponent(gpromise, filesToRewrite, replaceElement, match, saveDir);

                        } else {

                            insertHTMLComponentNextToElement(gpromise, filesToRewrite, !!beforeElement, beforeElement || afterElement, match, saveDir);
                        }
                    }
                ));
            ...
            gulp.src(path.resolve(PATHS.HTML_INPUT, from), {base: "./"})
                ...
                .pipe(
                    gReplace(getHTMLComponentRegExp(source), match => {
                        ...
                        insertHTMLComponent(gpromise, filesToRewrite, !!before, before || after, match, saveDir);
                    })
                );
            ...
            gulp.src(path.resolve(PATHS.HTML_INPUT, from), {base: "./"})
                ...
                .pipe(
                    gReplace(getHTMLComponentRegExp(source), match => {
                        ...
                        replaceHTMLComponent(gpromise, filesToRewrite, replace || source, match, saveDir);
                    })
                );
        }
            ...
            gulp.src(path.resolve(PATHS.HTML_INPUT, from), {base: "./"})
                ...
                .pipe(
                    gReplace(getHTMLSectionRegExp(source), (match, startTag, content) => {
                        ...
                        insertHTMLSection(gpromise, filesToRewrite, !!before, before || after, startTag + content, saveDir);
                    })
                );
            ...
            gulp.src(path.resolve(PATHS.HTML_INPUT, from), {base: "./"})
                ...
                .pipe(
                    gReplace(getHTMLSectionRegExp(source), (match, startTag, content) => {
                        ...
                        replaceHTMLSection(gpromise, filesToRewrite, replace || source, startTag + content, saveDir);
                    })
                );
        }
        ...
        if (afterComponent || beforeComponent || replaceComponent) {
                    ...
                    cheerio({
                        run: $ => {
                            ...
                            if (replaceComponent) {
                                ...
                                replaceHTMLComponentWithElement(gpromise, filesToRewrite, replaceComponent, content, comment, saveDir);

                            } else {

                                insertHTMLElementNextToComponent(gpromise, filesToRewrite, !!beforeComponent, beforeComponent || afterComponent, content, comment, saveDir);
                            }
                        },
                        parserOptions: {
            ...
            gulp.src(path.resolve(PATHS.HTML_INPUT, from), {base: "./"})
                .pipe(
                    cheerio({
                        run: $ => insertHTMLElement(gpromise, filesToRewrite, !!before, source, before || after, $(source), !!each, run, !noClosingComment, noNewLine, oppositeNewLine, saveDir),
                        parserOptions: {
                            normalizeWhitespace: false,
                            decodeEntities: false
                        }
            ...
            gulp.src(path.resolve(PATHS.HTML_INPUT, from), {base: "./"})
                .pipe(
                    cheerio({
                        run: $ => replaceHTMLElement(gpromise, filesToRewrite, source, replace, $(source), !!each, run, saveDir),
                        parserOptions: {
                            normalizeWhitespace: false,
                            decodeEntities: false
                        }
 ...
 */
function replaceHTMLComponent(gpromise, filesToRewrite, componentName, content, saveDir) {
    ...
    gulp.src(filesToRewrite, {base: "./"})
        ...
        .pipe(gpromise.deliverGulpPromise("copy"));
}
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function insertHTMLComponent(gpromise, filesToRewrite, insertBefore, componentName, content, saveDir) {
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function replaceHTMLSection(gpromise, filesToRewrite, sectionName, content, saveDir) {
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function insertHTMLSection(gpromise, filesToRewrite, insertBefore, sectionName, content, saveDir) {
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function replaceHTMLElement(gpromise, filesToRewrite, selector, replaceSelector, $content, useEach, runCode, saveDir) {
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function insertHTMLElement(gpromise, filesToRewrite, insertBefore, selector, targetSelector, $content, useEach, runCode, useClosingComment, noNewLine, oppositeNewLine, saveDir) {
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function insertHTMLElementNextToComponent(gpromise, filesToRewrite, insertBefore, componentName, content, comment, saveDir) {
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function replaceHTMLComponentWithElement(gpromise, filesToRewrite, componentName, content, comment, saveDir) {
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function insertHTMLComponentNextToElement(gpromise, filesToRewrite, insertBefore, selector, content, saveDir) {
 ...
 * saveDir (String): název složky pro uložení aktuálních souborů
 */
function replaceHTMLElementWithComponent(gpromise, filesToRewrite, selector, content, saveDir) {
...
function edit() {
    ...
    return gulp.src(filesToRewrite, {base: "./"})
        //uložit aktuální soubory
        .pipe(
            gulp.dest(path.resolve(PATHS.HTML_SAVE, `${yargs.argv.saveDir || Date.now()}`))
        )
...
exports.recoversave = recoversave;
...
function recoversave() {
    ...
    if (dirName && fs.statSync(path.resolve(PATHS.HTML_SAVE, dirName)).isDirectory()) {
        ...
        return gulp.src(path.resolve(PATHS.HTML_SAVE, dirName, "*.html"))
            .pipe(
                gulp.dest(path.resolve(PATHS.HTML_INPUT))
            );
    }

    return Promise.reject("Recovery directory not found!");
}
...
function clearsave() {
    ...
    if (yargs.argv.last) {
        ...
        return preserveOnlyLastHTMLSave();
    }

    return clearHTMLSave();
}
...
function getNewestDirName() {
    ...
    //Existuje složka s uloženými soubory?
    if (fs.existsSync(path.resolve(PATHS.HTML_SAVE)) && fs.statSync(path.resolve(PATHS.HTML_SAVE)).isDirectory()) {
 ...
 *
 * PostCSS přidá prefixy, media query převede na jednotky em a narhadí
 * CSS Custom Properties jejich hodnotami.
 *
...
const gPostcss = requireModule("gulp-postcss");
const postcss = requireModule("postcss");
...
function css(cb) {
    ...
    let cssLoad = requireUncached(path.resolve(PATHS.CSS_LOAD)),
        ...
        promises = Object.keys(cssLoad.files)
            .map(file => file + ":" + generateCSSBuild.name)
            .concat(
                Object.keys(cssLoad.files)
                    .map(file => file + ":" + generateCSSImport.name)
            ),
        gpromise = new gPromise();

    gpromise.makePromises(promises, () => cb());
    ...
    for (let file in cssLoad.files) {
        ...
        generateCSSBuild(gpromise, file, filesForFile);

        //Vytvoří soubor načítající CSS pomocí @import
        //pro testování při vypnutém JS
        generateCSSImport(gpromise, file, filesForFile);
    }

    return gpromise;
}

/* Spojí soubory z files do souboru file, použije PostCSS úpravy
 * a soubor vytvoří do výstupní složky pro CSS (PATHS.CSS_OUTPUT).
 *
 * gpromise (gPromise): gulp-promise
 * file (String): název výsledného souboru
 * files (Array[String]): pole vsupních souborů
 */
function generateCSSBuild(gpromise, file, files) {
    ...
    gulp.src(files.map(file => path.resolve(file)))
        ...
        .pipe(gPostcss([
            ...
            //zkonvertovat @media z px na em
            postcss.plugin("postcss-mqpxtoem", () => {

                let toFixed = (number, precision) => {
        ...
        .pipe(gpromise.deliverGulpPromise(file + ":" + arguments.callee.name));
 ...
 * files (Array[String]): pole vsupních souborů
 */
function generateCSSImport(gpromise, file, files) {
 ...
 *
 * Seznam modulů: gulp gulp-file yargs gulp-replace gulp-promise
 */
...
function comp(cb) {
    ...
    const MQS_SOURCE = path.resolve(PATHS.CSS_FILES, "utility.css"),
          COLORS_SOURCE = path.resolve(PATHS.CSS_FILES, "custom-properties.css");
    ...
    gulp.src(MQS_SOURCE)
        .pipe(
            ...
            gReplace(/[\s\S]*/, (match) => {
                ...
                let media = mq ? match.match(/(@media[^}{]+?\{)(?:\s*\#rewrite \.x-(?:desktop|tablet|mobile))/g) : [];

                media = media ? media.map(item => item.replace(/\{[\s\S]*/, "{}")) : [];

                gulp.src(COLORS_SOURCE)
                    .pipe(
                        //obsah dokumentu
                        gReplace(/[\s\S]*/, (match) => {
                            ...
                            colors = colors.map(color => color.match(/\:\s*([^\n\r;]+)/i)).map(color => color && color[1] ? color[1].trim() : null);

                            colors = colors.filter(Boolean)
                                        .map((color, c) => `${colorNames[c]}: ${color}`);
                            ...
                            file(fileName, template, {src: true})
                                .pipe(gulp.dest(dir))
                                .pipe(gpromise.deliverGulpPromise(name));
                        })
                    );
            })
        );
...
/* Zkompiluje soubory z ES2015+ do ES5 a spojují JS soubory podle
 * /dev/load-js.js (PATHS.JS_LOAD).
 * V tomto souboru je objekt FILES, kde se pro každý název souboru přiřadí
 * pole spojovaných souborů. Výsledné soubory se pak vytvoří do složky
 * PATHS.JS_OUTPUT. (Bude zde minifikovaná i neminifikovaná verze.)
 *
 * Seznam modulů: gulp gulp-rename gulp-uglify-es gulp-concat gulp-promise
 * gulp-babel
 * */

/*Minifikace JS.*/
const uglify = requireModule("gulp-uglify-es");
/*Převod ES2015+ do ES5*/
const babel = requireModule("gulp-babel");
...
function js(cb) {
    ...
    let jsLoad = requireUncached(path.resolve(PATHS.JS_LOAD)),
        ...
        promises = Object.keys(jsLoad.files),
        gpromise = new gPromise();
    ...
    for (let file in jsLoad.files) {
        ...
        generateJSBuild(gpromise, file, filesForFile);
    }
 ...
 * files (Array[String]): pole vsupních souborů
 */
function generateJSBuild(gpromise, file, files) {

    gulp.src(
            files.map(
                file => path.resolve(
                    file.replace(/^nobabel:/, "")
                        .replace(/^jsm:/, "")
                )
            )
        )
        ...
        .pipe(babel({
            ignore: files
                .filter(file => file.match(/^nobabel:/) !== null)
                .map(file => path.resolve(
                    file.replace(/^nobabel:/, "").replace(/^jsm:/, ""))
                ),
        ...
        .pipe(gpromise.deliverGulpPromise(file));
 ...
 * x-gns (String): bez globálního jmenného prostoru = neimportovat GNS.js
 * x-ns (String): bez lokálního jmenného prostoru
 * x-name (String): bez konstanty NAME (jméno modulu)
 * consts, const, c, objects, object, o (String): objekty na začátku modulu oddělené čárkou
 *     - c  = CLASS
 *     - i  = ID
 *     - d  = DATA
 *     - a  = ATTR
...
function mod() {
    ...
    let dir = yargs.argv.dir || yargs.argv.d || "",
        outputDir = path.resolve(PATHS.JS_MODULES, dir),
        fileName = `${name}.js`;

    //pokud modul existuje a není vynucené přepsání, ukončit
    if (!yargs.argv.rewrite && fs.existsSync(path.resolve(outputDir, fileName))) {
    ...
    const MODULE_CONSTS_ORDER = Object.keys(MODULE_CONSTS);

    let addJquery = !yargs.argv.xJq,
        addGNS = !yargs.argv.xGns,
        addNS = !yargs.argv.xNs,
        addName = !yargs.argv.xName,

        moduleConsts = (yargs.argv.consts || yargs.argv.const || yargs.argv.c || yargs.argv.objects || yargs.argv.object || yargs.argv.o || "")
            .split(",") //rozdělit podle čárky
            .filter(item => item.trim().length) //odstranit prázdné položky
            .sort((a, b) => //seřadit podle MODULE_CONSTS_ORDER
                MODULE_CONSTS_ORDER.indexOf(a.trim().toLowerCase()) - MODULE_CONSTS_ORDER.indexOf(b.trim().toLowerCase())
            )
            .sort(a => MODULE_CONSTS[a.trim().toLowerCase()] ? 0 : -1) //vlastní zařadit na konec
            .map(item => MODULE_CONSTS[item.trim().toLowerCase()] || item.trim()), //prevést na skutečné názvy

        depth = dir.split(/\/|\\/g).length + (dir ? 1: 0);

    let template = getJSModuleTemplate(name, addGNS, addName, addNS, addJquery, moduleConsts, depth);
 ...
 * useGNS (Boolean): importovat GNS.js,
 * usejQuery (Boolean): použit jQuery,
 * addName (Boolean): přidat konstantu NAME,
 * addNS (Boolean): přidat konstantu NS,
 * usejQuery (Boolean): použit jQuery,
 * useModuleVars (Array[String]): konstanty/objekty modulu
 * depth (Number): hloubka zanoření souboru
 */
function getJSModuleTemplate(name, useGNS, addName, addNS, usejQuery, useModuleConsts, depth) {

    const hasELEMENTandSELECTOR = useModuleConsts.includes("ELEMENT") && useModuleConsts.includes("SELECTOR"),

        lint = `/*jshint esnext: true, evil: true, browser: true, devel: true*/\n\n`,

        GNS = useGNS ? `import GNS from "${"../".repeat(depth)}GNS.js";\n\n` : "",
        NAME = addName ? `const NAME = "${name}";${addNS ? "\n": "\n\n"}`: "",
        NS = addNS && addName ? `const NS = \`${useGNS ? "${GNS}.": ""}\${NAME}\`;\n\n`: "",
        jQuery = usejQuery ? `const $ = ${useGNS ? "GNS": "window"}.jQuery;\n\n`: "",

        moduleConsts = `${useModuleConsts.map(item => `const ${item} = {
    ${item === "SELECTOR" ? "self: \"." + name.replace(/([A-Z])/g, "-$1").replace(/^-/, "").toLowerCase() + "\"" : ""}${item === "ELEMENT" ? "$self: null" : ""}
};`).join("\n\n")}${useModuleConsts.length ? "\n\n": ""}`,

        moduleFns = `function init() {

    if (!init.initialized) {
        ${hasELEMENTandSELECTOR ? "\n        ELEMENT.$self = $(SELECTOR.self);\n\n        if (ELEMENT.$self.length) {\n\n        }\n" : ""}
        init.initialized = true;
    }
}\n\n`,

        public = `export default { init };`;

    return `${lint}${GNS}${NAME}${NS}${jQuery}${moduleConsts}${moduleFns}${public}`;
}
...
function generate() {
    ...
    const gulpStream = gulp.src(filesToRewrite, {base: "./"})
        //uložit aktuální soubory
        .pipe(
            gulp.dest(path.resolve(PATHS.HTML_SAVE, `${yargs.argv.saveDir || Date.now()}`))
        )
...
function svg() {
    ...
    return gulp.src(`${PATHS.SVG_INPUT}/*.svg`)
        .pipe(svgSprite({
            shape: {
                spacing: {
                    padding: 0
 ...
 *
 * Seznam modulů: gulp critical yargs gulp-promise
 */

const gCritical = requireModule("critical");
...
function critical(cb) {
    ...
    gpromise.makePromises(pages, () => cb());

    //pro každý soubor vytvořit Critical CSS
    return Promise.all(pages.map(page => {

        return gCritical.generate({
...
function images() {
    ...
    return gulp.src([`${PATHS.IMG_INPUT}/**/*.{png,jpg,gif,svg}`])
        .pipe(imagemin([
